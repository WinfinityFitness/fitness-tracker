...
-- Affiliate single-URL fields (ad_settings.footer_webpage_url etc., see
...
-- Seed the 'social' category with the app's current hardcoded defaults so
-- the new popup isn't empty on first load — only runs if nothing's been
-- saved there yet (safe to re-run this migration without duplicating).
...
from (values
  ...
  ('Affiliate', 'http://www.facebook.com/aldruz3dsign', 3)
